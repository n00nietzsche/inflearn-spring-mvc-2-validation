...
//    @PostMapping("/add")
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        if(item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(new FieldError("item", "price", "가격은 1,000에서 1,000,000 사이여야 합니다."));
        }
        ...
        if(item.getPrice() != null && item.getQuantity() != null) {
            ...
            if(resultPrice < 10000) {
                bindingResult.addError(
                        ...
                        "가격 * 수량의 합은 10,000원 이상이어야 합니다. 현재 값은 " + resultPrice + " 입니다.")
                );
            }
        }
        ...
        if(bindingResult.hasErrors()) {
            // bindingResult 는 굳이 모델에 안담아도 스프링이 알아서 해준다.
    ...
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        ...
        if(!StringUtils.hasText(item.getItemName())) {
            bindingResult.addError(
                    // rejectedValue, bindingFailure 파라미터를 사용하기 시작했다.
                    new FieldError(
        ...
        if(item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(
                    new FieldError(
                            ...
                            false, null, null,
                            "가격은 1,000에서 1,000,000 사이여야 합니다."
                    )
            );
        }
